...
6)Использование параллельных коллекций
Using Concurrent Collections

.  ConcurrencyAPI включает в себя многочисленные классы коллекций , которые включают в себя встроенную поддержку для многопоточного, например,
ConcurrentHashMapи ConcurrentDeque. Он также включает в себя класс, CopyOnWriteArrayListкоторый создает копию своей базовой структуры списка каждый раз,
когда он изменяется, и полезен в средах с высокой степенью одновременности.

 Представляем параллельные коллекции
Первый вопрос, который вы можете задать: «Действительно ли нам нужны новые классы одновременной коллекции?» В конце концов, в предыдущем разделе вы видели,
что мы можем использовать synchronizedключевое слово в любом методе или блоке, поэтому не можем ли мы сделать то же самое для существующих классов коллекций?
Краткий ответ: «Мы могли бы». Например, взгляните на следующий код, который обращается Mapк synchronizedключевому слову:
...
Итак, зачем использовать класс одновременной коллекции? Как и использование ExecutorServiceдля управления потоками для нас,
использование параллельных коллекций чрезвычайно удобно на практике. Это также не позволяет нам вносить ошибки в собственную пользовательскую реализацию,
например, если мы забыли синхронизировать один из методов доступа. Фактически, одновременные коллекции часто включают улучшения производительности,
которые избегают ненужной синхронизации. Доступ к коллекциям из нескольких потоков настолько распространен, что создатели Java подумали, 
что было бы неплохо иметь альтернативные версии многих обычных классов коллекций только для многопоточного доступа.
Ниже приведена альтернативная версия нашей реализации, которая не использует synchronizedключевое слово, а вместо этого использует класс параллельной коллекции:
...
Вы можете заметить, что этот код почти идентичен нашей предыдущей реализации. Фактически, даже наш тип ссылки на объект Map, остался неизменным.
Как вы, возможно, помните из нашего обсуждения полиморфизма в Главе 2, даже если ссылочный тип изменяется, базовый объект по-прежнему является ConcurrentHashMap.
Также обратите внимание, что поскольку он ConcurrentHashMapреализуется Map, он использует те же методы get()/ put().
Поэтому ConcurrentHashMapв этом примере нет необходимости использовать ссылочный тип. 
